...
public class LoginModule : BaseModule 
{
    ...
    public void AccessServiceReq(int groupId, EnumAccessServiceType actionType, Action action)
    {
        ...
        siteId = groupId;
        NetController.Instance.SendAccessServiceReq(groupId, actionType,null, gameName);
    }

    //连接或断开服务器回应
    private void accessServiceAck(Message msg)
    {
        var rsp = msg.Content as CLGT.AccessServiceAck;
        switch (rsp.Errcode)
        {
            case 0:
            ...
            default:
               // var hintMsg = TLanguageHelper.DataMap.Values.ToList().Find(a => a.key == $"CLGTAccessServiceAck_{rsp.Errcode}").CN;
                GameController.Instance.CreateHintMessage(rsp.Errcode.ToString());
                break;
        }
    }
    ...
    {
        var rsp = msg.Content as CLGT.LoginAck;
        switch (rsp.Errcode)
        {
            case 0:
                ...
                AccessServiceReq(4, EnumAccessServiceType.JoinGame, () =>   // 登录成功 进入打地鼠游戏 gaimeID = 4
                {
                    AppRoot.Get().Init();
                });
                //UIManager.Instance.OpenUI(EnumUIType.LoadingUI, true, EnumUIType.MainUI, EnumSceneType.MainScene);
    ...
    {
        var rsp = msg.Content as CLGT.HandAck;
        switch (rsp.Errcode)
        {
            case 0:
                NetController.Instance.SendLoginPlatformReq(0);
                break;
            default:
    ...
    private void disconnectNtf(Message msg)
    {
        if (!SysDefines.IsDisconnect)
        {
            SysDefines.IsDisconnect = true;
            CoroutineController.Instance.StopAliveCor();
            var rsp = msg.Content as CLGT.DisconnectNtf;
