...
public abstract class BaseUI : MonoBehaviour
{
    ...
    public void Awake()
    {
       // print("BaseUI Awake GetUIType:"+ GetUIType().ToString());
        setAnimation();
    ...
    private void Start()
    {
      //  print("BaseUI Start GetUIType:" + GetUIType().ToString());
        if (UIOrder > -1)
        {
    ...
    {
        Func<bool> setParent = () => {
            var UIParent = GLuaSharedHelper.Get<RectTransform>("UIParent");
            if (UIParent) {
                CacheTransform.SetParent(UIParent, false);
                return true;
            }
            return false;
        };
        if (!setParent()) {
            Debug.LogError("setParent failed");
        }
      //  CacheTransform.SetParent(GlobalLuaShareWithCSharp.GetUnityObject<RectTransform>("UIParent"), false);
        State = EnumObjectState.Loading;
    }
