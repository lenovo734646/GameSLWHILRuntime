...
public class AssetUpdater : MonoBehaviour
{
    ...
    private string downLoadPath = "";   // 下载数据存储目录
    private string url = "";        // 下载源（可以是网络地址也可以是本地目录）
    private string listFile = "";

    public void StartUpdate(string url_, string listFile_, string downLoadPath_, string version)
    {
        url = url_;
        listFile = listFile_;
    ...
    private void StartCheckFileList()
    {
        ...
        StartCoroutine(Download(url+ listFile, null, OnFileListDone, OnError));
    }
    ...
    private void OnFileListDone(string text)
    {
        ...
        var curFileListPath = downLoadPath + listFile;
        ...
        foreach (var pair in newFileListMap)
        {
            ...
            var assetPath = UtilityEnv.StringConcat(pair.Value.filePath, "_", pair.Value.hash, UtilityEnv.Bundle_PostFix);

            FileNode node;
            if (oldFileListMap.TryGetValue(fileName, out node))
            {
                if (!node.hash.Equals(pair.Value.hash) || !node.length.Equals(pair.Value.length) || !File.Exists(downLoadPath+ assetPath))
    ...
    {
        if (!File.Exists(downLoadPath+path))
    ...
    private void TryLoadNextBundle()
    {
        ...
        StartCoroutine(Download(url + bundle, bundle, OnBundleDone, OnError));
    }
    ...
    private void OnAllLoadDone()
    {
        ...
        {
            var path = UtilityEnv.StringConcat(downLoadPath, bundle);
            if (File.Exists(path))
                File.Delete(path);
        }

        File.WriteAllText(UtilityEnv.StringConcat(downLoadPath, listFile), _loadFileList);
    ...
    private IEnumerator Download(string url, string savePath, Action<string> onDone, Action<int, string> onError)
    {
        ...
        {
            if (onError != null)
                onError(1, UtilityEnv.StringConcat(url, "\n", request.error));
        }
        else if (request.responseCode != 200)
        {
            if (onError != null)
                onError(2, UtilityEnv.StringConcat(url, "\nresponseCode:", request.responseCode.ToString()));
        }
        else
        {
            if (!string.IsNullOrEmpty(savePath))
            {
                string path = downLoadPath + savePath;
